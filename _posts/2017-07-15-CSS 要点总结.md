...
- 多行的行内元素：将父元素设置为display:table-cell和vertical-align:middle
...
### 水平垂直居中
- 已知宽高的元素：定位元素设置为position: absolute;top: 50%;left: 50%;margin-top和margin-left为元素高度一半的负值，父元素设置为position:relative;
- 未知宽高的元素：
1. 定位元素设置position: absolute;top: 50%; left: 50%; transform: translate(-50%, -50%);父元素设置为position:relative;
