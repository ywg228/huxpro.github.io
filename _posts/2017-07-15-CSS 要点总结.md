...
3. 设置position:relative 和 left:50%;
### 垂直居中
...
- 未知高度的块状元素：定位元素设置为position: absolute; top: 50%; transform: translateY(-50%); 父元素设置为position:relative;
### 水平垂直居中
- 已知宽高的元素：定位元素设置为position: absolute;top: 50%;left: 50%;margin-top和margin-left为元素高度一半的负值
- 未知宽高的元素：
