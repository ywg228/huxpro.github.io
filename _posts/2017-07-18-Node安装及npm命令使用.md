...
## 2. 环境变量配置
配置模块安装路径和缓存路径。 <br>
默认会将安装的模块安装到【C:\Users\用户名\AppData\Roaming\npm】路径中，占C盘空间。

#### 在Nodejs路径下创建两个文件夹 node_global 和 node_cache
#### 配置npm
第一种：在cmd中执行以下命令：这种方式安装模块时打开cmd时以管理员身份打开
```
...
在 用户变量 下的 Path中的 `C:\Users\用户名\AppData\Roaming\npm` 修改为 `C:\Program Files\nodejs\node_global`
